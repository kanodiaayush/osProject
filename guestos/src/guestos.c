...
void set_defaults(void) {
	...
	system(command);
	myblockSize = 8192;
    totalBlocks = heads*tracks*sectors*512/myblockSize;
    userBlockMap = malloc(totalBlocks*sizeof(int));
    long long int i;
    for(i = 0; i<totalBlocks; i++){
        userBlockMap[i] =-1;
    }
	 io_interrupt_queue = (struct io_interrupt **) malloc(sizeof(struct io_interrupt *) * INTERRUPT_SIZE);
	 instr_number = 0;
	 io_interrupt_queue_size = 0;
}
