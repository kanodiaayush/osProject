...
int handle_guest_syscalls() {
    ...
    {
        int new_slice = isa_regs->ebx;
        instr_slice = new_slice;
        isa_ctx->instr_slice = new_slice;
        retval=401;
        break;
    }
    case syscall_code_read_disk:
    {
    	//printf("\n%d %d %d %d %d\n",isa_regs->eax,isa_regs->ebx,isa_regs->ecx,isa_regs->edx,isa_regs->esi);
    	int numBytes = isa_regs->ebx;
	int memAddress = isa_regs->ecx;
	int blockNum = isa_regs->edx;
	int offset = isa_regs->esi;
	
	int myUid = isa_ctx->uid;
	
	//check userBlockMap
	int numBlocksSpanned;
	if(numBytes - (myblockSize-offset) > 0){
		numBlocksSpanned = ceil((double)(numBytes - (myblockSize-offset))/(double)myblockSize) + 1;
	}
	else{
		numBlocksSpanned = 1;
	}
	
	//printf("reading num blocks spanned %d\n", numBlocksSpanned);
	
	if(blockNum + numBlocksSpanned > totalBlocks || blockNum < 0){
		fatal("Disk space out of bounds\n");
	}
	
	long long int i;
	int garbage = 1;
	for(i = 0; i < numBlocksSpanned; i++){
		if(userBlockMap[blockNum + i] != myUid && userBlockMap[blockNum + i] != -1){
			fatal("Cannot read from disk. Space occupied by other user\n");
		}
		else if(userBlockMap[blockNum + i] != -1){
			garbage = 0;
		}
	}
	if(garbage == 1){
	...
	FILE * fp;
	fp = fopen("Sim_disk", "rb");
	if(fp == NULL){
		retval = -1;
	 	break;
	}
	fseek (fp ,blockNum*myblockSize, SEEK_SET );
	fread (buf , sizeof(char), numBytes, fp);
	fclose(fp);
	//printf("Buffer value is %s \n", buf);
	mem_write_string(isa_mem, memAddress, buf);
	...
	long long int i;
	for(i = 0; i < numBlocksSpanned; i++){
		if(!(userBlockMap[blockNum + i] == -1 || userBlockMap[blockNum + i] == myUid)){
			fatal("Cannot write to disk. Space occupied by other user\n");
		}
	}
	for(i = 0; i < numBlocksSpanned; i++){
		userBlockMap[blockNum + i] = myUid;
	}
	...
	int length = mem_read_string(isa_mem, memAddress,numBytes, buf);
	if (length >= numBytes){
	 	retval = -1;
	 	break;
	}
	//printf("Buffer value is %s %d \n", buf, sizeof(buf));
	FILE * fp;
	fp = fopen("Sim_disk", "wb");
	if(fp == NULL){
		retval = -1;
	 	break;
	}
